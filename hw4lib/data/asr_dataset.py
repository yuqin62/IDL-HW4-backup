...
class ASRDataset(Dataset):
    def __init__(
        ...
        subset_size = max(1, min(len(self.fbank_files), subset_size))
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        return (
            padded_feats.float(),
            padded_shifted.long() if padded_shifted is not None else None,
            padded_golden.long() if padded_golden is not None else None,
            feat_lengths.long(),
            transcript_lengths.long() if transcript_lengths is not None else None,
        )
