...
class SequenceGenerator:
    ...
    def generate_beam(
        ...
        seqs = x.unsqueeze(1).repeat(1, beam_width, 1)  # (B, K, T)
        ...
        for _ in range(self.max_length - init_len):
            ...
            logits = torch.stack(
                [self.score_fn(seqs[:, beam_idx, :]) for beam_idx in range(beam_width)],
                dim=1
            )  # (B, K, V)
            logits = self._apply_repeat_penalty(logits, seqs, repeat_penalty)
            log_probs = torch.log_softmax(logits / temperature, dim=-1)

            # Force finished beams to only emit EOS
            if finished.any():
                log_probs = log_probs.masked_fill(finished.unsqueeze(-1), float("-inf"))
                log_probs[..., self.tokenizer.eos_id] = torch.where(
                    finished,
                    torch.zeros_like(log_probs[..., self.tokenizer.eos_id]),
                    log_probs[..., self.tokenizer.eos_id]
                )

            candidate_scores = scores.unsqueeze(-1) + log_probs  # (B, K, V)
            flat_candidate_scores = candidate_scores.view(batch_size, -1)  # (B, K*V)

            top_scores, top_idx = torch.topk(flat_candidate_scores, k=beam_width, dim=-1)
            next_beam_idx = top_idx // vocab_size
            next_tokens = top_idx % vocab_size

            gather_idx = next_beam_idx.unsqueeze(-1).expand(-1, -1, seqs.size(-1))
            seqs = torch.gather(seqs, dim=1, index=gather_idx)
            seqs = torch.cat([seqs, next_tokens.unsqueeze(-1)], dim=-1)

            finished = torch.gather(finished, dim=1, index=next_beam_idx) | (next_tokens == self.tokenizer.eos_id)
            scores = top_scores
